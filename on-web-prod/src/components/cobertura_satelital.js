...
class Map extends React.Component
{
    ...
    {   
       ...
       // const key = 'AIzaSyCnmK6idgVg5q6Kt7AVacghzaSmaG3wENU';
        ...
        $.ajax ({
          url: "https://9f4eqobkma.execute-api.us-east-1.amazonaws.com/prod/hispasat/cobertura",
          type: "POST",
          data: JSON.stringify({ 
              "latitud":latitudajax.replace("(",""),
                      "longitud":lonajax.replace(")","")
          ...
          success: function( data, status){
              ...
              if(data.hispasat.hasCoverage){
                  for ( var i = 0, l = data.hispasat.coverage.length; i < l; i++ ) {

                      coberturaAltan+= "<p> Internet "+data.hispasat.coverage[i]+"</p>";
                       cobertura+="Internet "+data.hispasat.coverage[i]+ ", ";
                  }
              ...
              $("#contrue").attr("href","/contrata?tecnologia=satelital&latitud="+latitudajax.replace("(","")+"&longuitud="+lonajax.replace(")","")+"&cobertura="+cobertura);

              }else{
                  coberturaAltan+="Sin cobertura"
                  // $("#coordenadas_modal").html($("#latlon").val());
                  $("#señal_mapa_sin_modal").modal("show");
                  $("#confalse").attr("href","/contrata?tecnologia=satelital&latitud="+latitudajax.replace("(","")+"&longuitud="+lonajax.replace(")","")+"&cobertura=Sin Cobertura");
        ...
        return (
            <div id='map' className='contianer-fluid p-0'>
                <div className='map-top w-100 bg-primary text-light text-right'>
                    <span className='text-uppercase align-middle'>Cobertura Satélital</span>
